...
export async function POST(req: Request, { params }: { params: { id: string } }) {
    try {
        ...
        if (!INVENTORY_ROLES.has(userRole)) {
            return NextResponse.json({ error: "No autorizado para recibir transferencias" }, { status: 403 });
        }
        ...
        if (userRole === "SUPERVISOR" && (!userBranchId || transfer.targetBranchId !== userBranchId)) {
            return NextResponse.json({ error: "Solo puedes recibir transferencias destinadas a tu sucursal" }, { status: 403 });
        }
